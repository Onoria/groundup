...
export default function Match() {
  ...
  useEffect(() => {
    if (user?.publicMetadata?.role) {
      setIsQueued(!!user.publicMetadata.queued)
  ...
  const joinQueue = async (formData: FormData) => {
    const updates = new FormData()
    updates.append('queued', 'true')
    updates.append('queuedAt', Date.now().toString())
